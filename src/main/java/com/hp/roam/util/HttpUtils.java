...
//			.setSocketTimeout(60000).setConnectionRequestTimeout(10000).setProxy(proxy).build();
	 ...
	 * @return
	 */
	public static String getPathVariable(String param, String url,PublicContentType publicContentType) {
		CloseableHttpClient client = HttpClients.createDefault();
		CloseableHttpResponse response = null;
		try {
			HttpGet get = new HttpGet(url + "/" + param);
			if(publicContentType != null){
				get.setHeader("AppId", publicContentType.getAppId());
				get.setHeader("AppSecurity", publicContentType.getAppSecurity());
			}
			get.setConfig(requestConfig);
			response = client.execute(get);
			String result = EntityUtils.toString(response.getEntity());
	 ...
	 * @return
	 */
	public static Map<String, String> postJson(String params, String url,PublicContentType publicContentType) {
		...
		try {
			...
			post.setHeader("Content-type", "application/json");
			if(publicContentType != null){
				post.setHeader("AppId", publicContentType.getAppId());
				post.setHeader("AppSecurity", publicContentType.getAppSecurity());
			}
			post.setEntity(reqEntity);
			response = client.execute(post);
			String result = EntityUtils.toString(response.getEntity());
			rMap.put("result", result);
			rMap.put("status", response.getStatusLine().getStatusCode()+"");
			return rMap;
		} catch (Exception e) {
			e.printStackTrace();
		}
